...
class ConfigService:
    ...
    def parse_properties_content(self, content: str) -> dict:
        config = {}
    ...
    def parse_properties(self, filepath: str) -> dict:
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        return self.parse_properties_content(content)
    ...
    async def get_config(self) -> dict:
        if settings.is_remote_mode:
            ...
            res = await agent_coordinator.send_request("read_file", {"path": "server.properties"})
            if res.get("status") == "error":
                if "not found" in str(res.get("detail")).lower():
                    return {}
    ...
    async def save_config(self, db: AsyncSession, config: dict, user_id: int | None = None) -> dict:
        ...
        # 4. Generate text lines
        lines = ["# Minecraft server properties", "# Generated by Antigravity Panel"]
        ...
        if settings.is_remote_mode:
            from app.services.agent_coordinator import agent_coordinator
            res = await agent_coordinator.send_request("write_file", {"path": "server.properties", "content": content})
            if res.get("status") == "error":
                raise HTTPException(status_code=400, detail=res.get("detail"))
        else:
            ...
            # Generate text lines and write to .tmp file atomically in separate thread
